...
namespace eShopSolution.Application.Catalog.Products
{
    public class ManageProductService : IManageProductService
    {
        ...
        public async Task<List<ImageProductViewModel>> GetListImage(int productId)
        {
            var images = await _context.ProductImages.Where(x => x.ProductId == productId).ToListAsync();
            var result = new List<ImageProductViewModel>();
            foreach(var image in images)
            {
                result.Add(new ImageProductViewModel
                {
                    Id = image.Id,
        ...
        {
            var image = await _context.ProductImages.FindAsync(imageId);
            if (image == null) throw new EShopException($"Cannot find a product image with id: {imageId}");
            await _storageService.DeleteFileAsync(image.ImagePath);
